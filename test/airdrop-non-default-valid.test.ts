...
describe("Airdrop: Succesful merkle tree - non default", function () {
    ...
    describe("GIVEN an airdroppable contract was deployed and set up using a merkle tree with a duplicated receiver", function () {
        before(async function () {
            ...
            const deploy = createFixtureDeployContract(this.merkleTree.getRoot());
        ...
        describe("WHEN all of the proofs are presented", function () {
            before(async function () {
                this.txs = await Promise.all(
                    this.merkleTree.leaves.map( async( leaf: Leaf, i: number ) =>
                        this.tx = await this.airdrop.claim(leaf.receiver, leaf.amount, this.merkleTree.getProof(i))
                    )
                )
            });
            it("THEN all of the tokens are sent", async function () {
                ...
                const balances = await Promise.all( this.merkleTree.leaves.map( (leaf: Leaf) => this.token.balanceOf(leaf.receiver) ))
            ...
            it("THEN all of the leaves are marked as used", async function () {
                this.merkleTree.leaves.map( async(leaf: Leaf) => 
                    expect(await this.airdrop.isLeafUsed(hashLeaf(leaf))).to.be.true
                )
            });
            it("THEN all of the events signaling the transfer are emitted", async function () {
                await Promise.all (this.txs.map( (tx, i) =>
                    expect(tx).to.emit(this.token, 'Transfer')
                    .withArgs(ethers.constants.AddressZero, this.merkleTree.getLeaf(i).receiver, this.merkleTree.getLeaf(i).amount )
                ))
            });
            });
    });
    ...
    describe("GIVEN an airdroppable contract was deployed and set up using a merkle tree with an odd number of leafs", function () {
        ...
        describe("WHEN all of the proofs are presented", function () {
        before(async function () {
            this.txs = await Promise.all(this.merkleTree.leaves.map( async(leaf: Leaf, i: number) => 
                await this.airdrop.claim( leaf.receiver, leaf.amount, this.merkleTree.getProof(i))
            ))
        });
        it("THEN all of the tokens are sent", async function () {
            return await Promise.all(this.merkleTree.leaves.map( async(leaf: Leaf) =>
        ...
        it("THEN all of the events signaling the transfer are emitted", async function () {
            return Promise.all(
                this.txs.map( (tx: ContractTransaction, i: number) => 
                    expect(tx)
    ...
    describe("GIVEN an airdroppable contract was deployed and set up using a merkle tree with a non-power of two(but even) number of leaves", function () {
        ...
        describe("WHEN all of the proofs are presented", function () {
            ...
            it("THEN all of the leaves are marked as used", async function () {
                const usedArray = await Promise.all(this.merkleTree.leaves.map(leaf => this.airdrop.isLeafUsed(hashLeaf(leaf))));
